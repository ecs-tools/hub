...
export default function RebillingDashboard({ userRole }) {
  ...
  const [bucketFilter, setBucketFilter] = useState("fixable"); // fixable | all | <bucket>
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setMeta({ graceDays: data.grace_days, cutoff: data.cutoff });
      } catch (err) {
  ...
  // Rows in the active status view (before bucket/search) — drives bucket counts.
  ...
  const byBucket = useMemo(() => {
    const m = {};
    statusRows.forEach(r => { (m[r.bucket] = m[r.bucket] || { count: 0, amount: 0 }); m[r.bucket].count++; m[r.bucket].amount += r.total_unpaid; });
    return m;
  }, [statusRows]);
  ...
  const filtered = useMemo(() => {
    ...
    else if (bucketFilter !== "all") list = list.filter(r => r.bucket === bucketFilter);
    if (q) list = list.filter(r => `${r.client} ${r.county} ${r.service_codes} ${r.note || ""}`.toLowerCase().includes(q));
    return list;
  }, [statusRows, bucketFilter, search]);
  ...
  return (
    <div style={S.body}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14, alignItems: "center" }}>
        ...
        {BUCKETS.filter(b => byBucket[b.name]).map(b => {
          const active = bucketFilter === b.name;
          const info = byBucket[b.name];
          return (
            <button key={b.name} onClick={() => setBucketFilter(b.name)}
              style={{ border: `1.5px solid ${active ? b.color : "#e2e8f0"}`, background: active ? b.color : "white", color: active ? "#fff" : "#475569", borderRadius: 8, padding: "6px 11px", fontSize: 12.5, fontWeight: active ? 700 : 500, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 7 }}>
              <span style={{ width: 8, height: 8, borderRadius: "50%", background: active ? "#fff" : b.color }} />
              {b.name}<span style={{ fontWeight: 700 }}>{info.count}</span><span style={{ opacity: 0.85 }}>· {fmtMoney(info.amount)}</span>
            </button>
          );
        })}
      </div>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 14 }}>
        <input type="text" placeholder="Search client, county, code, or note…" value={search} onChange={e => setSearch(e.target.value)}
          style={{ border: `1.5px solid ${search ? "#3b82f6" : "#e2e8f0"}`, background: search ? "#eff6ff" : "white", borderRadius: 8, padding: "8px 12px", fontSize: 13, width: 300, fontFamily: "inherit", outline: "none" }} />
        <span style={{ fontSize: 12, color: "#64748b", marginLeft: "auto" }}>
          {shown.length < filtered.length ? `Showing ${shown.length} of ${filtered.length}` : `${filtered.length} groups`}
        </span>
      </div>
      ...
      <div style={{ ...S.card, padding: 0, overflow: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {shown.map((r) => (
              <tr key={r.item_key} style={{ opacity: r.status === "paid" ? 0.6 : 1 }}>
                <td style={{ ...S.td, fontWeight: 600, color: "#1a2d4d", minWidth: 150 }}>
                  {r.client}
            ...
            {shown.length === 0 && (
              <tr><td colSpan={6} style={{ padding: 40, textAlign: "center", color: "#94a3b8", fontSize: 14 }}>Nothing here — no items match this view.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 12, lineHeight: 1.5 }}>
        ...
        or <strong>In Progress</strong> while you work it; notes save automatically. Summer Youth (SYD/SYT) is private pay and excluded.
