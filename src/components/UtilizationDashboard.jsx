...
// Alert colors are SEMANTIC (red/yellow/green = how close to running out) and
// stay literal on purpose — the design tokens cover neutral chrome only.
const BADGE = (level) => ({
...
function ServiceRow({ svc }) {
  ...
  return (
    <div style={{ padding: "9px 0", borderBottom: "1px solid var(--bg-hover)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6, gap: 8, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-2)", background: "var(--bg-soft)", padding: "2px 7px", borderRadius: 4 }}>
            {svc.serviceLabel}
          </span>
        ...
        {svc.serviceEndDate && (
          <span style={{ fontSize: 11, fontWeight: !svc.ended && days !== null && days <= 14 ? 700 : 400, color: !svc.ended && days !== null && days <= 30 ? "#dc2626" : "var(--text-2)" }}>
...
function ClientCard({ client }) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${level === "red" ? "#fecaca" : level === "yellow" ? "#fde68a" : "var(--border)"}`,
      borderLeft: `4px solid ${borderColor}`,
    }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, color: "var(--text-1)" }}>{client.name}</div>
          <div style={{ fontSize: 11, color: "var(--text-2)", marginTop: 2 }}>{client.county}{client.waiverType ? ` · ${client.waiverType}` : ""}</div>
        </div>
...
export default function UtilizationDashboard() {
  ...
  // ── Parse + group rows into active clients ───────────────────────────────────
  const { activeClients } = useMemo(() => {
    ...
    return { activeClients: toArray(clientMap) };
  }, [rows]);
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1400, margin: "0 auto" }}>
        ...
        {[
          ...
          return (
            <button key={key} onClick={() => setLevelFilter(key)}
              style={{
                border: `1.5px solid ${active ? (level === "red" ? "#ef4444" : level === "yellow" ? "#eab308" : level === "green" ? "#22c55e" : "var(--accent)") : "var(--border)"}`,
                background: active ? (level === "red" ? "#fee2e2" : level === "yellow" ? "#fef9c3" : level === "green" ? "#dcfce7" : "var(--accent-soft)") : "var(--bg)",
                color: active ? (level === "red" ? "#b91c1c" : level === "yellow" ? "#854d0e" : level === "green" ? "#166534" : "var(--accent)") : "var(--text-2)",
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        <input
          ...
          style={{
            border: `1.5px solid ${nameSearch ? "var(--accent)" : "var(--border)"}`,
            background: nameSearch ? "var(--accent-soft)" : "var(--bg)",
        ...
        <select value={countyFilter} onChange={e => setCountyFilter(e.target.value)}
          style={{ border: "1.5px solid var(--border)", borderRadius: 8, padding: "7px 12px", fontSize: 13, fontFamily: "inherit", outline: "none", background: "var(--bg)", cursor: "pointer" }}>
          {counties.map(c => <option key={c}>{c}</option>)}
        </select>
        {hasFilters && (
          <button onClick={() => { setNameSearch(""); setCountyFilter("All"); }}
            style={{ border: "1.5px solid var(--border)", borderRadius: 8, padding: "7px 12px", fontSize: 13, background: "var(--bg)", cursor: "pointer", color: "var(--text-2)", fontFamily: "inherit" }}>
            Clear
          </button>
        )}
      </div>
